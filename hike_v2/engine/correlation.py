...
When multiple legs in a parlay come from the same game, their outcomes are
NOT independent. E.g., if the QB has a big passing day, the WR is likely
having a big receiving day. This module applies correlation discounts to
prevent overstating parlay probabilities.
"""
...
from config import (
    SGP_SAME_DIRECTION_CORR,
    SGP_OPPOSITE_DIRECTION_CORR,
    CROSS_GAME_CORR,
)

logger = logging.getLogger("parlay_assister.engine.correlation")
...
def adjust_parlay_probability(
    ...
    corr_factor = SGP_SAME_DIRECTION_CORR * same_game_pairs / total_pairs
...
def compute_correlation_matrix(legs: List[Dict]) -> np.ndarray:
    """
        ...
        for j in range(i + 1, n):
            ...
            if game_i and game_i == game_j:
                if _same_direction(legs[i], legs[j]):
                    corr[i, j] = corr[j, i] = SGP_SAME_DIRECTION_CORR
                else:
                    corr[i, j] = corr[j, i] = SGP_OPPOSITE_DIRECTION_CORR
            else:
...
def _same_direction(leg_a: Dict, leg_b: Dict) -> bool:
    ...
    if "pass" in type_a and "reception" in type_b:
        return True
    if "reception" in type_a and "pass" in type_b:
        return True
